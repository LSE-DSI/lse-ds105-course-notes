...
3. Walk around freely up and down the directories and sub-directories you encounter along the way. Check the links on [🔖 Week 02 Appendix - Linux and the Terminal](/weeks/week02/appendix.qmd) page to understand where you are navigating.
...
🎯 **ACTION POINTS** 
 ...
 You can use 'Homebrew' to install software in a very similar way to `apt` command in a Linux system.
...
To insert text characters, you need to switch to the `Insert Mode` by pressing `i`. In the left bottom, you can see --INSERT-- and start to write. You can shift back to 'Normal Mode' by pressing `ESC`.
...
- Vim initialises with `Normal Mode` by default in which you enter the commands but you cannot insert any character directly.  In normal mode;
    ...
    You can navigate your cursor with "h, j, k, l"  
    delete any line with "dd"  
    delete a character with "x"  
    insert a character after the cursor with 'a'  
    insert the previously deleted text after the cursor with 'p'    

    There are many "VIM Commands" waiting to be discovered by you.

- In `Insert Mode` you basically can enter your text/characters.

- You can also switch to `Visual Mode` by pressing 'v' while in normal mode, and you can manipulate text groups by cutting, copying, and deleting.
...
Check out the [🔖 Week 02 Appendix - 📃 Text editors](/weeks/week02/appendix.qmd) section for more info on `vim`.
